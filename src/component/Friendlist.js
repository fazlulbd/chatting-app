...
const Friendlist = (props) => {
  ...
  return (
    ...
    //     <h3 className='font-numito font-semibold text-xl'>Friend List</h3>
    //     {
    //       friend.map(item=>(
    ...
    //             {
    //               props.block ?
    //               <button onClick={()=>handleBlock(item)} className='font-numito font-semibold text-lg bg-primary text-white px-3 rounded'>Block</button>
    //               :
    //               <button onClick={()=>handleActiveChat(item)} className='font-numito font-semibold text-lg bg-primary text-white px-3 rounded'><RiMessage2Fill/></button>
    //             }                  
    //           </div>
    //         </div>
    //       ))
    ...
    <>
      ...
      {friend.map((item) => (
        <div onClick={() => handleActiveChat(item)} className="flex justify-between items-center border-b border-solid border-black pb-2.5 m-5" >
          <img src="images/profileimg.png" className="w-[70px] h-[70px] rounded" />
          <div>
